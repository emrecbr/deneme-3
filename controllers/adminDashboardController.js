...
export const getDashboardSummary = async (req, res, next) => {
  try {
    ...
    const [
      ...
      RFQ.countDocuments({ status: STATUS_OPEN }),
      RFQ.countDocuments({ status: STATUS_CLOSED }),
      RFQ.countDocuments({ status: 'awarded' }),
      RFQ.countDocuments({ status: STATUS_OPEN, moderatedAt: { $exists: false } })
    ]);
    ...
    const moderationQueue = await RFQ.find({ status: STATUS_OPEN, moderatedAt: { $exists: false } })
      .sort({ createdAt: -1 })
      .limit(6)
      .select('_id title status createdAt city district')
      .lean();
