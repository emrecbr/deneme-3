...
const EVENT_LABELS = {
  ...
  login_otp_send: 'Giris OTP gonderimi',
  login_otp_verify: 'Giris OTP dogrulama',
...
export default function AdminOtpLogs() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!active) return;
        setError(err?.response?.data?.message || 'OTP loglari alinamadi.');
      } finally {
  ...
  return (
    <div className="admin-panel">
      <div className="admin-panel-title">OTP Loglari</div>
      <div className="admin-panel-body">
        <div className="admin-info">
          Bu ekran dogrudan `/admin/notifications/otp-logs` endpointinden beslenir. Kayit varsa gercek OTP hareketlerini
          listeler; bos ise filtrelenen aralikta kayit yok demektir.
        </div>
        ...
        {!loading && items.length ? (
          <div className="admin-success">Gercek veri yansitiliyor: {items.length} OTP kaydi bulundu.</div>
        ) : null}
        {loading ? (
